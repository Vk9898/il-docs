...
const getDisclaimer = (language) => {
  ...
  return {
    ...
    content: `This document was translated from English to ${language} by InstaLaw using Google Gemini 2.5 Pro AI.
...
Prompt: "Translate the following legal document from English to ${language}, do not make omissions, do not fabricate falsehoods. Just translate in its entirety the provided document as best you can given the context."`
  };
};
...
// Create fixed PDF with proper formatting
async function createFixedPDF(translatedContent, language, outputPath) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const disclaimer = getDisclaimer(language.name);

      // First page - Disclaimer only
      // Simple header without background rectangle
      doc.fontSize(16)
         .fillColor('#003366')
         .font('Helvetica-Bold')
         .text('INSTALAW', 72, 72);
      ...
      doc.fontSize(9)
         .fillColor('#666666')
         .font('Helvetica')
         .text('Professional Legal Document Translation Services', 72, 95);
      
      // Language badge on the right
      doc.fontSize(10)
         .fillColor('#003366')
         .text(`${language.name} Translation`, 0, 72, {
           align: 'right',
      ...
      // Start translated content on same page if space, otherwise new page
      if (doc.y > 500) {
        doc.addPage();
      ...
      doc.fontSize(14)
         .fillColor('#000000')
         .font('Helvetica-Bold')
         .text('TRANSLATED LEGAL DOCUMENT', {
           align: 'center'
         });
      ...
      for (const paragraph of paragraphs) {
        ...
        if (doc.y > doc.page.height - 100) {
          ...
          doc.fontSize(8)
             .fillColor('#999999')
             .text(`Page ${pageNumber}`, 72, doc.page.height - 50, {
               align: 'center',
               width: doc.page.width - 144
             });
          ...
          doc.fontSize(8)
             .fillColor('#666666')
             .font('Helvetica')
             .text(`${language.name} Translation - Continued`, doc.page.width - 200, 50);
...
async function processLanguage(originalText, language) {
  ...
  try {
    ...
    // Create PDF
    await createFixedPDF(translation, language, pdfPath);
...
// Main function for all 40 languages
async function main() {
  try {
    ...
    for (let i = 0; i < LANGUAGES.length; i += batchSize) {
      ...
      console.log(`\nBatch ${Math.floor(i/batchSize) + 1}/${Math.ceil(LANGUAGES.length/batchSize)}`);
      console.log(`Processing: ${batch.map(l => l.name).join(', ')}\n`);
